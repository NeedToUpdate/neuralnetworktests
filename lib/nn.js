class NeuralNetwork {
  ...
  static fromJSON(json) {
    ...
    brain.num_of_outputs = temp.num_of_outputs;
    brain.num_of_inputs = temp.num_of_inputs;
...
class Layer {
  ...
  init() {
    ...
    switch (this.config.type) {
      case "lstm":
        ...
        let caches = [this.hidden_error_cache, this.cellstate_error_cache, this.cellstate_output_cache, this.forget_output_cache, this.i_output_cache, this.a_output_cache, this.outputs_calcd_cache];
  ...
  backpropagateCNN(input_errors) {
    ...
    for (let i = 0; i < this.config.feature_maps; i++) {
      ...
      for (let j = 0; j < this.num_of_inputs; j++) {
        ...
        let inp = this.input_value_cache[0][j][0].copy().flipX().flipY();
  ...
  backPropagateThroughTime(error_array) {
    ...
    for (let i = this.input_value_cache.length; i > 0; i--) {
      ...
      nexterrors = Matrix.multiply(wT, errors).flatten().slice(this.num_of_inputs);
    ...
    return Matrix.multiply(wT, errors).flatten().slice(0, this.num_of_inputs);
  }
  ...
  backPropagateThroughTimeLSTM(error_array) {
    ...
    for (n; n > 0; n--) {
      ...
      let c = new_hidden_errors
        ...
        .add(
          (this.cellstate_error_cache[this.config.time_steps] || new Matrix(Array(this.config.num_of_rns).fill(0))).multiply((this.forget_output_cache[n + 1] || new Matrix(Array(this.config.num_of_rns).fill(0))).map(activations["sigmoid"]))
        );
      ...
      let error_tanh_i = c.copy().multiply(this.i_output_cache[n].copy().map(activations["sigmoid"])).multiply(this.a_output_cache[n].copy().map(d_activations["tanh"]));
      let error_sigm_i = c.copy().multiply(this.a_output_cache[n].copy().map(activations["tanh"])).multiply(this.i_output_cache[n].copy().map(d_activations["sigmoid"]));
      let error_forget = c
        .copy()
      ...
      let error_output = new_hidden_errors.copy().multiply(this.cellstate_output_cache[n].copy().map(activations["tanh"])).multiply(this.outputs_calcd_cache[n].copy().map(d_activations["sigmoid"]));
